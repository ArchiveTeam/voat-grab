...
More information about the archiving project can be found on the ArchiveTeam wiki: [Voat](http://archiveteam.org/index.php?title=Voat)
...
Package `libzstd-dev` version 1.4.4 is required which is currently available from `buster-backports`.
    ...
    python3 -m pip install setuptools wheel
    python3 -m pip install --upgrade seesaw zstandard requests
    su -c "cd /home/archiveteam; git clone https://github.com/ArchiveTeam/voat-grab.git; cd voat-grab; ./get-wget-lua.sh" archiveteam
    screen su -c "cd /home/archiveteam/voat-grab/; run-pipeline3 pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam
    [... ctrl+A D to detach ...]
...
**There is a known issue with some packaged versions of rsync. If you get errors during the upload stage, voat-grab will not work with your rsync version.**
...
6. `screen su -c "cd /home/archiveteam/voat-grab/; run-pipeline pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam`
...
### For Alpine Linux:
    ...
    git clone https://github.com/ArchiveTeam/voat-grab
    cd voat-grab; ./get-wget-lua.sh
...
Have an issue not listed here? Join us on IRC and ask! We can be found at hackint IRC [#scrapevoat](https://webirc.hackint.org/#irc://irc.hackint.org/#scrapevoat).
